...
class XMPPService {
	...
	public function connect(): bool {
		...
		$password = $this->keyStore->get('xmpp_password');
	...
	public function handleIncomingMessage(Message $message): void {
		...
		$conversation = $this->findConversation($message);

		if ($conversation) {
			$this->relayMessageToTalk($message, $conversation);
		} else {
	...
	/**
	 * @param Message $message
	 * @return \OCA\Talk\Api\IRoom|null
	 */
	private function findConversation(Message $message): ?\OCA\Talk\Api\IRoom {
		$from = $message->getFrom();
		...
		try {
			if ($message->getType() === Message::TYPE_GROUPCHAT) {
				...
				if ($this->userManager->userExists($senderUsername)) {
					return $this->talkManager->getOne2OneRoom($senderUsername, $botUserId);
				}
			}
		} catch (\Exception $e) {
	...
	/**
	 * @param Message $message
	 * @param \OCA\Talk\Api\IRoom $conversation
	 */
	private function relayMessageToTalk(Message $message, \OCA\Talk\Api\IRoom $conversation): void {
		$from = $message->getFrom();
		$body = $message->getBody();
		$botUserId = $this->config->getAppValue('talk_xmpp_bridge', 'bot_user_id');
